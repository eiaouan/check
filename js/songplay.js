...
// 3,循环播放
(function(){
    let set = document.querySelector('.play-set');
    set.flag = 1;
...
// 顺序播放
function playOrder(ele) {
    //  index = 0;
    barSong.removeEventListener('ended',playRandomc); // 解绑
    barSong.removeEventListener('ended',setSrc);
    barSong.addEventListener('ended', playOrderc);  // 绑定
    if(ele){
        ele.style.backgroundPosition = '117px 273px';
    }
}
...
function playRandom(ele){
    barSong.removeEventListener('ended',playOrderc);
    barSong.removeEventListener('ended',setSrc);
    barSong.addEventListener('ended',playRandomc);
    ele.style.backgroundPosition = '60px 367px';
}

function playRandomc(){
    barSong.index = Math.floor(barSong.idList.length*Math.random())-1;
    barSong.sid = barSong.idList[barSong.index];
    setSrc();
}

// 循环播放
function playOne(ele){
    barSong.removeEventListener('ended',playOrderc);
    barSong.removeEventListener('ended',playRandomc)
    barSong.addEventListener('ended',setSrc);
    ele.style.backgroundPosition = '56px 272px';
}
...
function processLyric(date) {
    ...
    // 设置监听
    barSong.addEventListener('timeupdate', a);
    let div = document.querySelector('.lyric') // 显示歌词的地方
    div.addEventListener('mouseenter', function () {
        // 给给滚动事件解绑
        barSong.removeEventListener('timeupdate', a, false)
